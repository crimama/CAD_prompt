...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            use_e_prompt=False, e_prompt_layer_idx=[], use_prefix_tune_for_e_prompt=False, same_key_value=False, prototype_size=5, feature_layer = [5]):
        """
        Args:
...
def _create_vision_transformer(variant, pretrained=False, **kwargs):
    ...
    print(getattr(pretrained_cfg, 'url', ''))

    # Create model instance without loading weights
    # Pass pretrained_cfg so that model default_cfg is set.
    ...
    if pretrained:    
        url = getattr(pretrained_cfg, 'url', '')
        if url:  # Only proceed if there's a URL
            try:
                from timm.models.helpers import download_cached_file
                checkpoint_path = download_cached_file(url)
                
                # Check if file is actually a .npz file by examining content
                import os
                if checkpoint_path.endswith('.npz') or 'npz' in url:
                    # Handle .npz files using our custom loader
                    print(f"Loading .npz weights from {checkpoint_path}")
                    _load_weights(model, checkpoint_path)
                else:
                    # Try to determine file type by checking if it's a valid PyTorch file
                    try:
                        import torch
                        ...
                        # Apply filter function if needed
                        state_dict = checkpoint_filter_fn(state_dict, model)
                        
                        # Load state dict
                        missing_keys, unexpected_keys = model.load_state_dict(state_dict, strict=False)
                        print(f"Loaded PyTorch weights. Missing keys: {len(missing_keys)}, Unexpected keys: {len(unexpected_keys)}")
                        
                    except Exception as torch_error:
                        print(f"Failed to load as PyTorch file: {torch_error}")
                        # Last resort: try as .npz even if extension doesn't match
                        try:
                            print(f"Attempting to load as .npz file: {checkpoint_path}")
                            _load_weights(model, checkpoint_path)
                        except Exception as npz_error:
                            print(f"Failed to load as .npz file: {npz_error}")
                            raise RuntimeError(f"Could not load weights from {url}. PyTorch error: {torch_error}, NPZ error: {npz_error}")
                    
            except Exception as e:
                print(f"Warning: Failed to load pretrained weights from {url}: {e}")
