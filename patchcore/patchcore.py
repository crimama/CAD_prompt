...
class PatchCore(torch.nn.Module):
    ...
    def load(
        ...
        **kwargs,
        ...
        # backbone은 더 이상 사용하지 않으므로 None으로 처리
        self.backbone = None
        ...
        # backbone을 사용하지 않으므로 feature_aggregator는 더미로 설정
        # 실제로는 ViT의 self.model과 self.prompt_model을 사용
        feature_aggregator = None  # 사용하지 않음
        feature_dimensions = [768]  # ViT의 기본 차원
        ...
        self.model = load_weights_from_npz(self.model, 'vit_base_patch16_224')
        self.prompt_model = load_weights_from_npz(self.prompt_model, 'vit_base_patch16_224')
    ...
    def _embed(self, images, detach=True, provide_patch_shapes=False):
        ...
        # feature_aggregator는 사용하지 않음 (ViT 사용)
        with torch.no_grad():
    ...
    def save_to_path(self, save_path: str, prepend: str = "") -> None:
        ...
        patchcore_params = {
            "backbone.name": "vit_base_patch16_224",  # ViT 모델 사용
    ...
    def load_from_path(
        ...
        # backbone은 사용하지 않으므로 None으로 설정
        patchcore_params["backbone"] = None
        if "backbone.name" in patchcore_params:
